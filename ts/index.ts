...
type constructor_option = {
    changeTime: number      // 时间 s
}

class Carousel {
    img_data: img_obj[]         // 图片数据
    readonly length: number     // 整个轮播图的图片数量
    readonly changeTime: number // 轮播图的播放下一张图片的时间
    private timer: number       // 定时器ID
    private index: number = 0   // 当前展示图片的下标
    private firstIndex: number = 0 // 上一张图片的下标
    private flag: boolean = false

    private img_box: HTMLElement
    private small_dot: HTMLElement
    private caption: HTMLElement


    constructor(useNode: HTMLElement, img_data: img_obj[], option: constructor_option = {changeTime: 4}) {
     ...
     * */
    private init(domNode: HTMLElement): void {
        ...
        this.small_dot.addEventListener('click',  (e) => {
            if (e.target !== this.small_dot) {
                this.stop()
                this.index = Number((e.target as any).dataset.index)
                this.change()
                this.Interval()
            }
        })
        ...
        domNode.addEventListener('mousemove', () => {
            (this.img_box.children[this.index] as HTMLElement).style.transform = 'scale(1.05)'
            button_box.style.opacity = `1`;
            this.stop()
        })
        domNode.addEventListener('mouseout', () => {
            (this.img_box.children[this.firstIndex] as HTMLElement).style.transform = ''
    ...
    private change() {
        ...
        //清除上一张图片的样式
        const firstDOMElement = <HTMLElement>this.img_box.children[this.firstIndex]
        ...
        const captionChildren: any = this.caption.children
